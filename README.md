...
docker run --rm --name minio-standalone \
  -e 'MINIO_ACCESS_KEY=my_key' \
  -e 'MINIO_SECRET_KEY=my_long_secret_key' \
  -v $PWD/data/minio:/data \
  -p 9000:9000 \
  wtayyeb/minio-rancher:latest
```
...
- **MINIO_ACCESS_KEY**: The access key to connect on Minio.
- **MINIO_SECRET_KEY**: The secret key to connect on Minio.
